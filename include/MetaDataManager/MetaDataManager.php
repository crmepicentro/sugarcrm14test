...
class MetaDataManager implements LoggerAwareInterface
{
    ...
    private const BEFORE_FAULT_CONFIG_KEY = 'locks.iterations_before_fault.MetaDataManager::getMetadataInternal';
    /**
     * @var DBManager|mixed
     */
    public $db;
    ...
    protected static $configProperties = [
        ...
        'default_currency_show_preferred' => true,
        ...
        'default_time_format' => true,
        'email_default_client' => true,
    ...
    public function getServerInfo()
    {
        ...
        $data['db_name'] = DBManagerFactory::getInstance()->dbName;
    ...
    protected function getConfigs()
    {
        ...
        // If system email is not configured to allow outbound access to users, default client should not be 'sugar'
        $oe = BeanFactory::newBean('OutboundEmail');
        if ($configs['emailDefaultClient'] === 'sugar' && !$oe->isAllowUserAccessToSystemDefaultOutbound()) {
            $configs['emailDefaultClient'] = 'mailto';
        }

        // Default order-able system tabs
        $configs['systemTabs'] = [
            'display' => array_values(array_diff($this->getTabList(), ['Home'])),
        ];

        // SugarPDF default settings
        foreach (SugarAutoLoader::existingCustom('include/Sugarpdf/sugarpdf_default.php') as $file) {
            require $file;
        }
        $configs['defaultPdfFontNameMain'] = $sugarpdf_default['PDF_FONT_NAME_MAIN'];
        $configs['defaultPdfFontSizeMain'] = $sugarpdf_default['PDF_FONT_SIZE_MAIN'];
        $configs['defaultPdfFontNameData'] =  $sugarpdf_default['PDF_FONT_NAME_DATA'];
        $configs['defaultPdfFontSizeData'] = $sugarpdf_default['PDF_FONT_SIZE_DATA'];
    ...
    protected function loadAndCacheMetadata($args, MetaDataContextInterface $context, $ignoreCache = false)
    {
        ...
        if ($metadataRebuildRequired) {
            ...
            $hash = (string)$context->getHash() . intval($this->public);
            $systemProcessLock = new SystemProcessLock(
                __METHOD__,
                $hash,
                [
                    'iterations_before_fault' => SugarConfig::getInstance()->get(self::BEFORE_FAULT_CONFIG_KEY, 180),
                ]
            );
            ...
            $longRunningFunction = function (int $attempt) use (&$data, $args, $context, $oldHash) {
