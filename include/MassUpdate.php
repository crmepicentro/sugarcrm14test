...
class MassUpdate
{
    ...
    */
    ...
    public function getMassUpdateFormHeader($multi_select_popup = false)
    {
        ...
        if ($multi_select_popup) {
            ...
            $tempString = '<form action="index.php" method="post" name="MassUpdate" id="MassUpdate" onsubmit="return check_form(\'MassUpdate\');">'
                ...
                . '<input type="hidden" name="lvso" value="' . htmlspecialchars((string)$lvso, ENT_QUOTES, 'UTF-8') . '" />'
                . '<input type="hidden" name="' . htmlspecialchars($order_by_name, ENT_QUOTES, 'UTF-8') . '" value="' . htmlspecialchars((string)$request_order_by_name, ENT_QUOTES, 'UTF-8') . '" />'
        ...
        if ($module == 'Emails') {
            ...
            // determine owner
            $tempString .= "<input type='hidden' name='type' value='" . htmlspecialchars((string)$type, ENT_QUOTES, 'UTF-8') . "' />\n"
     ...
     */
    public function handleMassUpdate($fetch_only = false, $update_blank = false)
    {
        ...
        if (!empty($_REQUEST['uid']) && !isset($_REQUEST['entire'])) {
            $_POST['mass'] = $this->request->getValidInputRequest(
                'uid',
                ['Assert\Delimited' => ['constraints' => 'Assert\Guid']]
            );
            ...
            $query = $this->sugarbean->create_new_list_query($order_by, $this->where_clauses, [], [], 0, '', false, $this, true, true);
        ...
        if (isset($_POST['mass']) && is_array($_POST['mass']) && $_REQUEST['massupdate'] == 'true') {
            ...
            foreach ($_POST['mass'] as $id) {
                ...
                if (isset($_POST['Delete'])) {
                    ...
                    if ($this->sugarbean->ACLAccess('Save', ['massupdate' => true])) {
                        $_POST['record'] = $id;
                        $_GET['record'] = $id;
                        $_REQUEST['record'] = $id;
    ...
    {
        $banned = ['date_modified' => 1, 'date_entered' => 1, 'created_by' => 1, 'modified_user_id' => 1, 'deleted' => 1, 'modified_by_name' => 1,];
        ...
        } elseif ($moduleName == 'InboundEmail') {
    ...
    public function getMassUpdateForm(
        ...
        static $banned = ['date_modified' => 1, 'date_entered' => 1, 'created_by' => 1, 'modified_user_id' => 1, 'deleted' => 1, 'modified_by_name' => 1,];
                            ...
                            if (!empty($field['isMultiSelect'])) {
                                if (!empty($field['function'])) {
                                    $functionValue = getFunctionValue(
                                        ...
                                        $field['function'],
                                        [$this->sugarbean, $field['name'], '', 'MassUpdate']
                                    );
        ...
        if (isset($teamhtml)) {
            ...
            if (!in_array($teamhtml, ['<tr>', '</tr>', '<tr></tr>', '<tr><td></td></tr>'])) {
                $html .= $teamhtml;
            }
...
EOHTML;
     ...
     * @param varname name of the variable
     */
    public function addTeamList($displayname, $field)
    {
...
EOQ;
    ...
    public function addDate($displayname, $varname)
    {
        ...
        $calDateformatJs = Escape::js($cal_dateformat);
        $javascriptend = <<<EOQ
		 <script type="text/javascript">
		Calendar.setup ({
			inputField : "{$varname}jscal_field", daFormat : "$calDateformatJs", ifFormat : "$calDateformatJs", showsTime : false, button : "{$varname}jscal_trigger", singleClick : true, step : 1, weekNumbers:false
		});
		</script>
EOQ;
        $jscalendarImage = SugarThemeRegistry::current()->getImageURL('jscalendar.gif');
        $calDateformatJsHtmlAttr = Escape::html($calDateformatJs);
        $userFormatHtml = Escape::html($userformat);
        $html = <<<EOQ
	<td scope="row" width="20%">$displayname</td>
	<td class='dataField' width="30%"><input onblur="parseDate(this, '$calDateformatJsHtmlAttr')" type="text" name='$varname' size="12" id='{$varname}jscal_field' maxlength='10' value="">
    <img src="$jscalendarImage" id="{$varname}jscal_trigger" align="absmiddle" title="{$app_strings['LBL_MASSUPDATE_DATE']}" alt='{$app_strings['LBL_MASSUPDATE_DATE']}'>&nbsp;<span class="dateFormat">$userFormatHtml</span>
	$javascriptend</td>
...
EOQ;
    ...
    public function addDatetime($displayname, $varname)
    {
        ...
        $calDateformatJs = Escape::js($cal_dateformat);
        $calDateformatJsHtmlAttr = Escape::html($calDateformatJs);
        $userFormatJs = Escape::js($userformat);

        $javascriptend = <<<EOQ
		 <script type="text/javascript">
		Calendar.setup ({
			inputField : "{$varname}_date",
			daFormat : "$calDateformatJs",
			ifFormat : "$calDateformatJs",
			showsTime : false,
...
EOQ;
        ...
        $html = <<<EOQ
		<td scope="row" width="20%">$displayname</td>
		<td class='dataField' width="30%"><input onblur="parseDate(this, '$calDateformatJsHtmlAttr')" type="text" name='$varname' size="12" id='{$varname}_date' maxlength='10' value="">
		...
		$dtscript
		<script type="text/javascript">
		var combo_{$varname} = new Datetimecombo(" ", "$varname", "$userFormatJs", '','','',1);
