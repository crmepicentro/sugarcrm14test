...
EOF;
    ...
    private array $modifyModulesExcludeActions = [
        'ModuleBuilder' => ['exportpackage'],
    ];
    ...
    protected function isModifyAction()
    {
        ...
        if (isset($this->modifyModules[$this->controller->module])) {
            if ($this->modifyModules[$this->controller->module] === true) {
                return empty($this->modifyModulesExcludeActions[$this->controller->module])
                    || (
                        is_array($this->modifyModulesExcludeActions[$this->controller->module])
                        && !safeInArray($action, $this->modifyModulesExcludeActions[$this->controller->module])
                    );
            }
