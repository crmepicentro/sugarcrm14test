...
(function(app) {
    app.events.on("app:init", function() {
        ...
        function prepopulateEmailForCreate(email, data) {
            ...
            if (data.outboundEmails) {
                app.utils.removeOutboundEmails(email.attributes.to_collection, data.outboundEmails);
                app.utils.removeOutboundEmails(email.attributes.cc_collection, data.outboundEmails);
                app.utils.removeOutboundEmails(email.attributes.bcc_collection, data.outboundEmails);
            }
        ...
        app.utils = _.extend(app.utils, {
             ...
             * @param {Array} outboundEmails
             */
            removeOutboundEmails: function(collection, outboundEmails) {
                let emails = [];
                if (_.isObject(collection)) {
                    emails = collection.models.map(collection => collection.attributes.email_address);
                }
                outboundEmails.forEach((emailAddress) => {
                    const index = emails.indexOf(emailAddress);
                    if (index !== -1) {
