...
class ModuleInstaller
{
    ...
    public function pre_execute()
    {
        ...
        if (isset($this->installdefs['pre_execute']) && is_array($this->installdefs['pre_execute'])) {
            foreach ($this->installdefs['pre_execute'] as $includefile) {
                require_once FileLoader::validateFilePath(str_replace('<basepath>', $this->base_dir, $includefile));
            }
        }
    }
    ...
    {
        foreach ($this->extensions as $extname => $ext) {
            if (empty($filter) || safeInArray($extname, $filter)) {
 ...
 */
    ...
    public static function getPortalConfig()
    {
        ...
        $portalConfig = [
            ...
            'serverUrl' => $config->get('site_url') . '/rest/v11_24',
            ...
            switch ($action) {
                case 'Restore':
                    ...
                    foreach ($installed_modules as $module) {
                        if (safeInArray($module, $currentTabs)) {
