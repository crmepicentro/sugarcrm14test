...
*}
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<td ><span sugar='slot5b'><input id='start_date' name='wiz_step1_start_date' title='{$MOD.LBL_CAMPAIGN_START_DATE}' onblur="parseDate(this, '{$CALENDAR_DATEFORMAT|escape:'javascript'|escape:'html'}');"  type="text" size='11' maxlength='10' value="{$CAMP_START_DATE|escape:'html'}"> {sugar_getimage name="jscalendar" ext=".gif" alt=$APP.LBL_ENTER_DATE other_attributes='align="absmiddle" id="start_date_trigger" '} <span class="dateFormat">{$USER_DATEFORMAT|escape:'html'}</span></span sugar='slot'></td>
		...
		<td ><span sugar='slot7b'><input id='end_date' name='wiz_step1_end_date'  aria-required="true" title='{$MOD.LBL_CAMPAIGN_END_DATE}' onblur="parseDate(this, '{$CALENDAR_DATEFORMAT|escape:'javascript'|escape:'html'}');"  type="text"s size='11' maxlength='10' value="{$CAMP_END_DATE|escape:'html'}"> {sugar_getimage name="jscalendar" ext=".gif" alt=$APP.LBL_ENTER_DATE other_attributes='align="absmiddle" id="end_date_trigger" '} <span class="dateFormat">{$USER_DATEFORMAT|escape:'html'}</span></span sugar='slot'></td>
	...
	<script type="text/javascript">
		Calendar.setup ({
			inputField : "start_date", ifFormat : "{$CALENDAR_DATEFORMAT|escape:'javascript'}", showsTime : false, button : "start_date_trigger", singleClick : true, step : 1, weekNumbers:false
			
		});
		
		Calendar.setup ({
			inputField : "end_date", ifFormat : "{$CALENDAR_DATEFORMAT|escape:'javascript'}", showsTime : false, button : "end_date_trigger", singleClick : true, step : 2, weekNumbers:false
