...
{
    $GLOBALS['log']->fatal("Deprecated function get_worksheet_defintion called.");
    $lv = null;
    ...
    if (strtolower($forecast_type) == 'direct') {
        ...
        $totals = ['NAME' => translate('LBL_TOTAL_VALUE', 'Forecasts'), 'REVENUE' => $opp_summary['TOTAL_AMOUNT'], 'PROBABILITY' => '&nbsp;', 'WEIGHTED_VALUE' => $opp_summary['WEIGHTEDVALUE'], 'WK_BEST_CASE' => $opp_summary['TOTAL_WK_BEST_CASE'], 'WK_LIKELY_CASE' => $opp_summary['TOTAL_WK_LIKELY_CASE'], 'WK_WORST_CASE' => $opp_summary['TOTAL_WK_WORST_CASE'],];
...
function add_downline($user, &$parent_node)
{
    $ret = [];
    $node = null;
    if (safeCount($user->my_direct_reports) > 0) {
...
function create_node($user_id, $href = true, $dynamic = true, $force_direct = false, $top_level = false, $commit = false)
{
    ...
    if ($force_direct or safeCount($user->my_direct_reports) == 0) {
        $rollup = false;
    }
    if ($top_level) {
        if ($rollup) {
...
function buildExportLink($forecast_type)
{
    ...
    $buttons = ["<a href='javascript: void(0);'>" . $mod_strings['LBL_COPY'] . '</a>'];
