...
function create_field_lang_pak_contents($old_contents, $key, $value, $language, $module)
{
    if (!empty($old_contents)) {
        $old_contents = preg_replace("~[^\[\n\r]+\[" . preg_quote(var_export($key, true), '~') . "\].*';$~im", '', $old_contents);
        $contents = str_replace("\n?>", "\n\$mod_strings[" . var_export($key, true) . "] = " . var_export($value, true) . ";\n?>", $old_contents);
    } else {
        $module = preg_replace('~\W~is', '', $module);
        $language = preg_replace('~\W~is', '', $language);
        $contents = "<?php\n"
...
function save_custom_dropdown_strings($new_dd_strings, $language = '', $all_languages = false, $postponeQRR = false)
{
    ...
    if ($all_languages) {
        ...
        $languages = [$language => $language]; //need the array key defined
    }
...
function dropdown_item_move_up($dropdown_type, $language, $index)
{
    ...
    if ($index > 0 && $index < safeCount($dropdown_array)) {
        $key = '';
        $value = '';
        $i = 0;
...
function dropdown_item_move_down($dropdown_type, $language, $index)
{
    ...
    if ($index >= 0 && $index < safeCount($dropdown_array) - 1) {
        $key = '';
        $value = '';
        $i = 0;
...
function helper_dropdown_item_insert(&$dropdown_array, $index, $key, $value)
{
    ...
    if ($index >= safeCount($dropdown_array)) {
