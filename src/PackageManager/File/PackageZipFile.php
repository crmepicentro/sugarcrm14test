...
class PackageZipFile
{
    ...
    protected function patchCode(): void
    {
        $recursiveIterator = new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator(
                $this->packageDir,
        ...
        foreach (new RegexIterator($recursiveIterator, $regex) as $phpFile) {
            $code = file_get_contents($phpFile);
            $translatedCode = SweetTranslator::patch($code);
            if (null === $translatedCode) {
                continue;
            }
            if (false === sugar_file_put_contents($phpFile, $translatedCode)) {
                throw new \RuntimeException("Failed to write patched code into $phpFile");
            }
        }
    }
}
