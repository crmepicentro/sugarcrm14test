...
function checkDBSettings($silent = false)
{
    ...
    if (!empty($sugar_config['installer_locked'])) {
        die($mod_strings['LBL_DISABLED_DESCRIPTION']);
    }
...
function printErrors($errors)
{
    ...
    } elseif ((safeCount($errors) == 1 && (isset($errors['ERR_DB_EXISTS_PROCEED']) || isset($errors['ERR_DB_EXISTS_WITH_CONFIG'])))
        || (safeCount($errors) == 2 && isset($errors['ERR_DB_EXISTS_PROCEED']) && isset($errors['ERR_DB_EXISTS_WITH_CONFIG']))) {
